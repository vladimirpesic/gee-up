...
class RequestNew extends Component {
  ...
  onSubmit = async event => {
    ...
    try {
      ...
      await geeUp.methods.createRequest(description, web3.utils.toWei(value, 'ether'), recipient).send({
        from: accounts[0]
      });
      Router.pushRoute(`/geeups/${this.props.address}/requests`);
    } catch (err) {
  ...
  render() {
    return (
      <Layout>
        <Link route={`/geeups/${this.props.address}/requests`}><a>Back</a></Link>
